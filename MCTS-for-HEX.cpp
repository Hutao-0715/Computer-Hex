...
struct TreeNode{
    ...
    void rollout(){
        ...
        while(1){
            ...
            node->state.gpm();
            //cout << "pm = " << node->state.pm.size() << " total_move = " << node->state.total_move << "\n";
...
string hex_solver(vector<string> in_moves);
...
int main(){                                         //
    ...
    vector<string> v;
    //read file
    for(string str; f >> str;){v.push_back(str);}
    hex_solver(v);
                                                    //
    return 0;                                       //
}                                                   //
...
string hex_solver(vector<string> in_moves){ //input the previous moves, this function will give you the best next move
    int i,j,player=1;
    TreeNode* now = new TreeNode;
    string res;
    //set up
    ...
    res = cr2str(MCTS(now,5000)->this_move);
    //output
    cout << res << "\n";
